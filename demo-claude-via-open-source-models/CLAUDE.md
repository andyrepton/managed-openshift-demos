...
- **LLMInferenceService baseRefs**: KServe uses "differential versioning" ([opendatahub-io/kserve#1919](https://github.com/opendatahub-io/kserve/pull/1919)) — if a new version's preset content is identical to the previous version, no new config is created and the old name is preserved. On clusters **upgraded** from RHOAI 3.4, `v3-4-4` names (e.g. `v3-4-4-kserve-config-llm-template-nvidia-cuda`) still exist as preserved artifacts. On **fresh** RHOAI 3.5 installs, only `v3-5-0` names exist (e.g. `v3-5-0-kserve-config-llm-single-node-template-nvidia-cuda`, `v3-5-0-kserve-config-llm-router-route`). Always run `oc get llminferenceserviceconfig -A` to check which names are available on your cluster
